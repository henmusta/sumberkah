...
class BulananNopolJoController extends Controller
{

    function data(Request $request, $type){
        $bulan = ($type == 'post') ?   $request['bulan'] :  explode(',',  $request['bulan']);
        $mobil =  ($type == 'post') ?   $request['mobil_id'] :  explode(',',  $request['mobil_id']);
        ...
        foreach($mobil as $key => $val){
            $get_mobil = Mobil::findOrFail($val);
            ...
            ->whereYear('tgl_joborder', $tahun)->where('mobil_id', $val);
        }
        return $data;
    }
    public function index(Request $request)
    {
        $config['page_title'] = "Laporan Joborder Nopol";
        $page_breadcrumbs = [
          ['url' => '#', 'title' => "Laporan Joborder Nopol"],
        ];
        return view('backend.bulanannopoljo.index', compact('config', 'page_breadcrumbs'));
    }
    ...
    public function pdf(Request $request)
    {
        ...
        // dd($data);
        $pdf =  PDF::loadView('backend.bulanannopoljo.pdf',  compact('data'));
    ...
    public function getreport(Request $request)
    {
          ...
          return view('backend.bulanannopoljo.report', compact('data'));
   ...
   public function excel(Request $request)
   {
       ...
       $data = [
        ...
        $sheet->setCellValue('A1', 'Bulanan Nopol Joborder');
     ...
     $filename = 'Bulanan Nopol Joborder';
